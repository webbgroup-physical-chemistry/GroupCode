...
if __name__=="__main__":
    try : 
        ...
        begin, end, step, beta = sys.argv[2:6]
    except :
        print "usage: ./script <file name> <begin> <end> <step> <beta>"
        sys.exit()
...
##  This is a debugging exercise.  Here I guarantee that the counts are symmetric, rather than relying on the sampling method to guarantee that.  In conclusion, there is no bug and any asymmetry seen is due to the expected symmetric counts being asymmetric
##    newcounts=[]
##    for count in range(len(counts)/2) :
##        newcounts.append(int(counts[count]))
##    number=0
##    for count in range(len(counts)/2-1,-1,-1) :
##        
##        newcounts.append(int(counts[count]))
##    counts=newcounts
