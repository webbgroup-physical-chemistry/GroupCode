...
#define NFILE asize(fnm)
    ...
    do {
        ...
        double nitrile_angle_acos = iprod(xaxis, projected_vector)/( iprod(xaxis, xaxis) * iprod(projected_vector, projected_vector));
        if ( nitrile_angle_acos >= 1.0 )
        {
            phi=0.0;
        }
        else if ( nitrile_angle_acos <= -1.0 )
        {
            phi=180.;
        }
        else
        {
            ...
            if (projected_vector[1] >=0) { nitrile_angle = acos( nitrile_angle_acos )*180/M_PI ;}
            else { nitrile_angle = -1*acos( nitrile_angle_acos )*180/M_PI;}
