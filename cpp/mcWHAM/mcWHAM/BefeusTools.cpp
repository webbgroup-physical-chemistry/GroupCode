...
void Experiment::readtrajectory( std::string trajectory, double begin, double end, double bin_sizes, bool bVerbose )
{
    ...
    if ( file.is_open() ) {
        while (file.good()) {
            ...
            std::istream_iterator<std::string> it(check_line);
            std::istream_iterator<std::string> endline;
            size_t members = 0;
            while (it++ != endline) members++;
...
{
    std::stringstream string_check( line ) ;
        
    std::istream_iterator<std::string> it(string_check);
    std::istream_iterator<std::string> endline;
    size_t values = 0;
    while (it++ != endline) values++;
